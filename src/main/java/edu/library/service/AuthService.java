...
public class AuthService {
    ...
    public Roles addUser(String username, String password, String roleName) {
        ...
        Roles newUser = new Roles(username, password, roleName);
        users.add(newUser);
        saveUsersToFile();
        return newUser;
    }
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 3) {
                    ...
                    users.add(new Roles(username, password, roleName));
                }
            }
        } catch (IOException e) {
    ...
    private void saveUsersToFile() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            for (Roles user : users) {
                writer.write(String.format("%s,%s,%s%n", user.getUsername(), user.getPassword(), user.getRoleName()));
            }
        } catch (IOException e) {
